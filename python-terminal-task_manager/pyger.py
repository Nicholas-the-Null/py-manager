if __name__ == '__main__':
    ...
    all_command_name_list=["deamon","load","echo","secret","hs","hy","history","update","sudo","pwd","protection","unprotection","wifi","cd","changedirectory","chd",
    ...
    try:
        ...
        print(f"[+] success {nice_conta_file}")
        nice_conta_file+=1
        import deamon as deamon
        import preserve as preserve
        preserve_list=[]
    ...
    load_var=False
    history_path=r''+correct_path+"\\"+"history.txt"
    ...
    if os.path.exists(history_path):
        with open(history_path,"r") as file:
            ...
            history=list(filter(lambda x:x!='\n',history))
        file.close()
            ...
            if command[0].lower() in ["hs","hy","history"]:#history of command
                ...
                if status_deamon!=None:
                    ...
                    if len(command)==4:
                        action=command[0]
                        command.pop(0)
                        if action=="checkprocess":
                            status_deamon=multiprocessing.Process(name='p', target=deamon.monitor_process, args=(command[0],command[1],command[2],))
                            status_deamon.start()
                            print("deamon started")
                        elif action=="checkfile":
                            status_deamon=multiprocessing.Process(name='p', target=deamon.monitor_file, args=(command[0],command[1],command[2],))
                            status_deamon.start()
                            print("deamon started")
                        elif action=="checkfolder":
                            status_deamon=multiprocessing.Process(name='p', target=deamon.monitor_path, args=(command[0],command[1],command[2],))
                            status_deamon.start()
                            print("deamon started")
                        else:
                            console.print("[red]error invalid command"+"[/red]")
                ...
                if len(command)>=2:
                    ...
                    filename=command[1]
                    if os.path.isdir(path_command):
                        if os.path.isfile(r''+path_command+"\\"+filename):
                            with open(r''+path_command+"\\"+filename,"r",encoding="utf-8") as f:
                                content=f.read()
                            f.close()
                            filea=preserve.File(filename,content,path_command)
                            console.print("[green]file saved"+"[/green]")
                            save_debug("save","file saved")
                        else:
                            console.print("[red]file not found"+"[/red]")
